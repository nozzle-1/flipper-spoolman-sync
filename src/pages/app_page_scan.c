...
static void app_page_scan_format_hex(
    const uint8_t* bytes,
    size_t bytes_len,
    char* output,
    size_t output_size) {
...
static uint16_t app_page_scan_read_le_u16(const uint8_t* bytes) {
    return (uint16_t)bytes[0] | ((uint16_t)bytes[1] << 8);
}

static float app_page_scan_read_le_f32(const uint8_t* bytes) {
    union {
        uint32_t u32;
        float f32;
    } value;
...
static void app_page_scan_copy_ascii(
    char* output,
    size_t output_size,
    const uint8_t* bytes,
    size_t bytes_len) {
...
static void app_page_scan_draw_page_0(Canvas* canvas, const NfcScanResult* result) {
    ...
    app_page_scan_format_hex(result->blocks[9], NFC_BLOCK_SIZE, tray_uid, sizeof(tray_uid));
    app_page_scan_copy_ascii(detailed_type, sizeof(detailed_type), result->blocks[4], 16);
    app_page_scan_format_hex(result->blocks[5], 4, color_hex, sizeof(color_hex));
    app_page_scan_copy_ascii(material_id, sizeof(material_id), &result->blocks[1][8], 8);
...
static void app_page_scan_draw_page_1(Canvas* canvas, const NfcScanResult* result) {
    ...
    app_page_scan_format_hex(&result->blocks[16][4], 4, second_color_hex, sizeof(second_color_hex));
...
void app_page_scan_draw(Canvas* canvas, const SpoolmanSyncApp* app) {
    ...
    if(app->status == AppStatusScanSuccess) {
        app_page_scan_draw_header(canvas, app);
        switch(app->read_tag_page) {
        case 0:
            app_page_scan_draw_page_0(canvas, &app->scan_result);
            break;
        case 1:
            app_page_scan_draw_page_1(canvas, &app->scan_result);
            break;
        case 2:
            app_page_scan_draw_page_2(canvas, &app->scan_result);
            break;
        default:
            app_page_scan_draw_page_3(canvas, &app->scan_result);
            break;
        }
        return;
    }
